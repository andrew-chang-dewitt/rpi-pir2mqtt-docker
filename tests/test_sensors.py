...
def test_default_sensor_returns_TRIPPED_if_rising(default_sensor):
    assert default_sensor.determine_state(factories.mock_gpio_input_rising_fn) == 'TRIPPED'

def test_motion_sensor_returns_TRIPPED_if_rising(motion):
    assert motion.determine_state(factories.mock_gpio_input_rising_fn) == 'TRIPPED'

def test_window_sensors_return_TRIPPED_if_falling(window):
    assert window.determine_state(factories.mock_gpio_input_falling_fn) == 'TRIPPED'

def test_door_sensors_return_TRIPPED_if_falling(door):
    assert door.determine_state(factories.mock_gpio_input_falling_fn) == 'TRIPPED'
